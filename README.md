# instagaram-downloader
The application, whose gui has been built using tkinter, uses the intrascape API to download content from instagram


- First we import the needed libraries
```python
from tkinter import *
from PIL import ImageTk, Image
import tkinter.font as font
from tkinter import messagebox
from instascrape import Reel
import time
```
- Here the funtion to dounload is defined and the scrapper is inistialized.

```python
#Dowload button function add also intitialize the scrapper.
def download(link):
    try:
        if link:
            SESSIONID = "Enter sessionid here"
            headers ={
                "User-Agent" : "Mozilla/5.0 (Windows NT 10.0; WIN64: x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945"
                               "Safari/537.36 Edg/79.0.309.43",
                 "cookie" : f"sessionId = {SESSIONID}"
            }

            google_reel = Reel(link)
            google_reel.scrape(headers = headers)
            google_reel.download(fp = f".\\reel {int(time.time())}.mp4")
            messagebox.showinfo("Status", "Reel downloaded Successfully")

        else:
            messagebox.showwarning("Empty Field" , "Please Enter the link")
    except Exception as e :
        messagebox.showerror("Error", "Something went wrong, Please try again later")
        print(e)
```

- Now we set up our Graphical User Interface using tkinter and configure it to our preferred settings and arrange code how we'd like the widgets to appear on the screen.


```python

#setting up the GUI
window = Tk()
window.title("Instagam Photo/Video Downloader")
window.minsize(600, 500)
window.maxsize(600, 500)

#Defining global variables
WEIGHT = 500
WIDTH = 600
FONT = font.Font(family="Times New Roman", size="18", weight="bold")
FONT1 = font.Font(family="Times New Roman", size="12", weight="bold")
FONT3 = font.Font(family="Times New Roman", size="10", weight="bold")
TEXT ="1. Only public account reels can be downloaded.\n "\
    "2. Enter the link address of the reel from the instagram account"


canvas = Canvas(window, height=WEIGHT, width=WIDTH)
canvas.pack()

frame = Frame(window, bg="white")
frame.place(relwidth= 1, relheight= 1)

background_image = ImageTk.PhotoImage(Image.open(r"images.jpeg"))
background_label = Label(frame, image = background_image)
background_label.place(relx=-0.25, relwidth= 0.7, relheight= 1)

label1 = Label(frame, text = "Download Instagram Content", font=FONT, bd= 5, fg="#0d1137", bg="white")
label1.place(relx = 0.48, rely= 0.1, relheight= 0.1)

label2 = Label(frame, text=(" Enter Link Address : "), font=FONT1, bd= 5, fg = "#052165", bg="white")
label2.place(relx= 0.48, rely= 0.25, relheight= 0.1)

entry = Entry(frame, font=FONT1, fg = "#fbad50")
entry.place(relx=0.48, rely=0.35, relwidth= 0.4, relheight=0.05)

button1 = Button(window, text = "Download", font=FONT1, bg="pink", fg="black", activeforeground= "pink", activebackground="black", command=download)
button1.place(relx = 0.48, rely = 0.45, relwidth=0.2, relheight=0.06)

label2 = Label(frame, text="Instructions: ", font=FONT1, bd=5, fg="black", bg="white")
label2.place(relx=0.48, rely=0.6, relheight=0.1)

label2 = Label(frame, text=TEXT, font=FONT3, bd=5, fg = "#cd486b", justify=LEFT, bg="white")
label2.place(relx=0.48, rely=0.7, relheight=0.1)

window.mainloop()

```
